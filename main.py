...
selected_name = st.sidebar.selectbox("Select Name", all_names, index=0)
if selected_name != 'All':
    filtered_df = df[df['Gender'] == selected_name]
else:
    filtered_df = df

filtered_df['Total Experience better than Average'] = np.where(filtered_df['Total years of experience'] > filtered_df['Total years of experience'] .mean(), 'Yes', 'No')


all_workmode= ['All'] + list(filtered_df['Total Experience better than Average'].unique())
selected_gender = st.sidebar.selectbox("Employee having More Experience than Average", all_workmode, index=0)
if selected_gender != 'All':
    filtered_df = filtered_df[filtered_df['Total Experience better than Average'] == selected_gender]
